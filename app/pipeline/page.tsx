...
export default function PipelinePage(){
  ...
  const loadAll=async(userId:string)=>{
    ...
    (noteData||[]).forEach(n=>{if(!nm[n.project_id])nm[n.project_id]=[];nm[n.project_id].push(n);});
    setNotes(nm);
    setActivity(actData||[]);
    setLoading(false);
  };
  ...
  const addNote=async()=>{
    ...
    setSavingNote(true);
    const{data}=await supabase.from("notes").insert({project_id:selectedProject.id,created_by:user.id,content:newNote.trim()}).select().single();
    if(data){
  ...
  return(
    <div style={{height:"100vh",background:"var(--bg)",color:"var(--text)",fontFamily:"var(--font-body)",display:"flex",flexDirection:"column",overflow:"hidden"}}>
      ...
      <div style={{padding:"14px 16px 12px",flexShrink:0,borderBottom:"1px solid var(--border)"}}>
        ...
        {/* Stats strip — single row, scrollable */}
                  ...
                  {(notes[selectedProject.id]||[]).length===0
                    ...
                    :(notes[selectedProject.id]||[]).map(note=>(
                      <div key={note.id} className="note-item">
                        ...
                        <div style={{fontSize:13,lineHeight:1.6,whiteSpace:"pre-wrap"}}>{note.content}</div>
                      </div>
                    ))
                  }
                </div>

...
function AppraisalPage(){
  ...
  const sensMatrix=sensitivity(); if(sensMatrix&&r) (r as any).sensMatrix=sensMatrix;
  ...
  const runStaticChecks=useCallback(()=>{
    ...
    // Jurisdiction-aware DSCR floor — EUR markets accept 1.20×, UAE/India push 1.30×
    const _jurisdiction=getJurisdictionProfile(data.currency);
    const dscrFloor=_jurisdiction.dscrFloor;
    ...
    if(assetType==="BTR"){
      ...
      if(isFinite(r.dscr)){
        if(r.dscr<dscrFloor)flags.push({severity:"error",field:"DSCR / ICR",message:`DSCR of ${r.dscr?.toFixed(2)}× is below the ${dscrFloor.toFixed(2)}× lender covenant minimum for ${data.currency||"GBP"} markets.`,benchmark:`${data.currency||"GBP"} senior lenders require ICR ≥ ${dscrFloor.toFixed(2)}×`});
    ...
    if(assetType==="Hotel"){
      ...
      if(isFinite(r.dscr)){
        if(r.dscr<dscrFloor)flags.push({severity:"error",field:"DSCR / ICR",message:`DSCR of ${r.dscr?.toFixed(2)}× is below the ${dscrFloor.toFixed(2)}× minimum for ${data.currency||"GBP"} markets.`,benchmark:`${data.currency||"GBP"} hotel lenders require ICR ≥ ${dscrFloor.toFixed(2)}×`});
    ...
    if(assetType==="Flip"){
      ...
      if(data.flipCapStructure==="bridge_refi"&&(data.holdOccupancy||"vacant")==="tenanted"&&isFinite(r.dscr)&&r.dscr>0){
        if(r.dscr<dscrFloor)flags.push({severity:"error",field:"DSCR / ICR",message:`DSCR of ${r.dscr.toFixed(2)}× is below the ${dscrFloor.toFixed(2)}× lender covenant minimum for ${data.currency||"GBP"} BTL/refi — refi may not be approvable at this rent level.`,benchmark:`${data.currency||"GBP"} BTL/refi lenders require DSCR ≥ ${dscrFloor.toFixed(2)}× (stress-tested: +0.10–0.25× over floor)`});
          ...
          <div style={{padding:24,overflowY:"auto",flex:1}} className="editor-pad">
            ...
            {activeTab==="revenue"&&assetType==="Hotel"&&(
              <div>
                ...
                {/* ── Hotel Simple — Normalisation Assumptions panel ─────────────
                   Surfaces the margin + FF&E structure applied when Normalised
                   mode drives EBITDA/NOI. Users can tune all three fields here
                   without leaving the Revenue tab. In Advanced mode these are
                   managed per-year in the IM & Costs tab. */}
                {hotelMode==="simple"&&(
                  <div style={{background:"var(--bg2)",border:"1px solid var(--border)",borderRadius:10,padding:14,marginBottom:16}}>
                    <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:10}}>
                      <div style={{fontSize:11,fontWeight:600,color:"var(--text-m)",textTransform:"uppercase",letterSpacing:".08em"}}>Normalisation Assumptions</div>
                      <div style={{fontSize:10,color:"var(--text-d)",fontStyle:"italic"}}>institutional defaults — override for deal-specific structures</div>
                    </div>
                    <div className="inp-row-3">
                      <div className="inp-group">
                        <label className="inp-label">Rooms GOP Margin (%)</label>
                        <input className="inp" type="number" step="1" value={data.roomsMarginPct??""} onChange={e=>set("roomsMarginPct",e.target.value)} placeholder="75"/>
                        <div style={{fontSize:9,color:"var(--text-d)",marginTop:3}}>Typical: 70–80% for 4–5 star institutional assets</div>
                      </div>
                      <div className="inp-group">
                        <label className="inp-label">F&B GOP Margin (%)</label>
                        <input className="inp" type="number" step="1" value={data.fnbMarginPct??""} onChange={e=>set("fnbMarginPct",e.target.value)} placeholder="30"/>
                        <div style={{fontSize:9,color:"var(--text-d)",marginTop:3}}>Typical: 25–35% after labour + cost of goods</div>
                      </div>
                      <div className="inp-group">
                        <label className="inp-label">FF&E Reserve (% of revenue)</label>
                        <input className="inp" type="number" step="0.5" value={data.ffePct??""} onChange={e=>set("ffePct",e.target.value)} placeholder="3"/>
                        <div style={{fontSize:9,color:"var(--text-d)",marginTop:3}}>Industry standard: 3% of total revenue</div>
                      </div>
                    </div>
...
${cf>=0?"+":"-"}${currencySymbol}${Math.abs(Math.round(cf)).toLocaleString()}`}
                ...
                <div style={{background:"var(--bg2)",border:"1px solid var(--border)",borderRadius:12,padding:20,marginBottom:20}}>
                  ...
                  {assetType==="Hotel"&&(()=>{
                    ...
                    return([
                      ...
                      [hotelMode==="advanced"?"Interest (Paid Over Hold)":"Interest (Rolled)",fmt(r.interestCost||0,currencySymbol),"var(--amber)"],
        ...
        <div className="output-panel" style={{padding:20,position:"sticky",top:0,height:"calc(100vh - 102px)",overflowY:"auto",background:"var(--bg1)",display:panelOpen?"block":"none"}}>
          ...
          <div style={{background:"var(--bg2)",border:"1px solid var(--border)",borderRadius:10,padding:14,marginBottom:16}}>
            ...
            {assetType==="Hotel"&&([{label:"Purchase + Property Tax",value:(r.purchasePrice||0)+(r.sdlt||0),color:"var(--text-m)"},{label:"CapEx",value:r.capex,color:"var(--text-m)"},...((r.vat||0)>0?[{label:vatLabel(data.currency||"GBP"),value:r.vat,color:"var(--amber)"}]:[]),{label:"Arrangement Fee",value:r.arrangementFee,color:"var(--amber)"},{label:hotelMode==="advanced"?"Interest (Paid Over Hold)":"Interest (Rolled)",value:r.interestCost,color:"var(--amber)"},{label:"Total Investment",value:r.totalInvestment,color:"var(--gold)",bold:true}] as any[]).map((item:any)=>(<div key={item.label} style={{display:"flex",justifyContent:"space-between",padding:"5px 0",borderBottom:"1px solid var(--bg4)",fontSize:12}}><span style={{color:"var(--text-m)"}}>{item.label}</span><span style={{fontFamily:"var(--font-mono)",color:item.color,fontWeight:item.bold?600:400}}>{fmt(item.value||0,currencySymbol)}</span></div>))}
            {assetType==="Flip"&&([

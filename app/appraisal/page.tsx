...
:root{
  /* ── Valora Dark Mode (default) ── */
  --gold:#c9993c;--gold-l:#d9b060;--gold-bg:rgba(201,153,60,0.08);--gold-border:rgba(201,153,60,0.22);
  --bg:#0f1115;--bg1:#131720;--bg2:#171a1f;--bg3:#1d2128;--bg4:#232830;--bg5:#2a2e34;
  --text:#f5f5f5;--text-m:#a6a6a6;--text-d:#5a5e64;
  --border:rgba(255,255,255,0.07);--border-m:rgba(255,255,255,0.13);
  --green:#1f8f5f;--red:#b44545;--amber:#c98a1e;--blue:#4a80c4;
...
body.light{
  /* ── Valora Light Mode ── */
  --gold:#c9993c;--gold-l:#a07828;--gold-bg:rgba(201,153,60,0.08);--gold-border:rgba(201,153,60,0.28);
  --bg:#f7f6f3;--bg1:#f0efe9;--bg2:#ffffff;--bg3:#f7f6f3;--bg4:#e2e0da;--bg5:#d5d3cc;
  --text:#0f1115;--text-m:#4a4f55;--text-d:#9a9a96;
  --border:#e2e0da;--border-m:#c8c6be;
  --green:#1f8f5f;--red:#b44545;--amber:#c98a1e;--blue:#2d6aa0;
}
body.light .cell-r{background:rgba(180,69,69,.09);color:var(--red)}
body.light .cell-a{background:rgba(201,138,30,.09);color:var(--amber)}
body.light .cell-g{background:rgba(31,143,95,.09);color:var(--green)}
...
.btn-primary{background:var(--gold);color:#0f1115;border:none;border-radius:7px;padding:10px 20px;font-family:var(--font-body);font-size:13px;font-weight:600;cursor:pointer;transition:background .2s;display:inline-flex;align-items:center;gap:8px}
    ...
    <div className="inp-group" style={{gridColumn:"1 / -1"}}>
      ...
      <div style={{background:"rgba(201,168,76,0.07)",border:"1px solid rgba(201,168,76,0.2)",borderRadius:7,padding:"8px 12px",marginBottom:10,display:"flex",alignItems:"flex-start",gap:8}}>
        <svg width="14" height="14" viewBox="0 0 14 14" fill="none" style={{flexShrink:0,marginTop:2}} xmlns="http://www.w3.org/2000/svg">
          <circle cx="7" cy="7" r="6.5" stroke="#c9993c" strokeWidth="0.75"/>
          <rect x="6.4" y="6" width="1.2" height="4.5" rx="0.6" fill="#c9993c"/>
          <circle cx="7" cy="4.2" r="0.7" fill="#c9993c"/>
        </svg>
...
async function generatePDF(data:any,results:any,assetType:string,currencySymbol:string,userEmail:string){
  ...
  // Valora palette — institutional, print-friendly
  const gold=[201,153,60] as [number,number,number];    // #C9993C brand gold
  const dark=[247,246,243] as [number,number,number];   // #F7F6F3 ivory background
  const bg2=[255,255,255] as [number,number,number];    // white cards
  const bg3=[247,246,243] as [number,number,number];    // ivory alternate rows
  const bg4=[226,224,218] as [number,number,number];    // #E2E0DA border tone
  const grey=[74,79,85] as [number,number,number];      // #4A4F55 secondary text
  const white=[15,17,21] as [number,number,number];     // #0F1115 primary text
  const green=[31,143,95] as [number,number,number];    // #1F8F5F muted positive
  const red=[180,69,69] as [number,number,number];      // #B44545 muted negative
  const amber=[201,138,30] as [number,number,number];   // #C98A1E muted warning
  const blue=[45,106,160] as [number,number,number];    // muted blue
  ...
  const pageHeader=(doc:any,subtitle:string)=>{
    ...
    drawVMarkPDF(doc,M,3,7,gold);
    doc.setTextColor(...gold);doc.setFontSize(9);doc.setFont("helvetica","bold");doc.text("VALORA",M+9,10);
  ...
  drawVMarkPDF(doc,M,8,10,gold);
  doc.setTextColor(...gold);doc.setFontSize(20);doc.setFont("helvetica","bold");doc.text("VALORA",M+13,20);
...
async function generateBrochurePDF(data:any,r:any,assetType:string,currencySymbol:string,content:BrochureContent,photos:string[],hotelMode="simple",hotelAdv:any=null){
  ...
  drawVMarkPDF(doc,M,brandY,8,gold);
  doc.setTextColor(...gold);doc.setFontSize(12);doc.setFont("helvetica","bold");doc.text("VALORA",M+11,brandY+9);
  doc.setTextColor(...grey);doc.setFontSize(6.5);doc.setFont("helvetica","normal");
  doc.text(isHotelAdv?"INSTITUTIONAL INVESTMENT MEMORANDUM":"INVESTMENT MEMORANDUM",M+11,brandY+15);
  ...
  let py2=20;
  drawVMarkPDF(doc,M,py2-7,7,gold);
  doc.setTextColor(...gold);doc.setFontSize(9);doc.setFont("helvetica","bold");doc.text("VALORA",M+9,py2);
  ...
  sections.forEach(([title,key])=>{
    ...
    py2=wrapText(doc,content[key]||"",M,py2,W-M*2,5.5);
    py2+=4;
  });
  ...
  // Header
  drawVMarkPDF(doc,M,13,7,gold);
  doc.setTextColor(...gold);doc.setFontSize(9);doc.setFont("helvetica","bold");doc.text("VALORA",M+9,20);
  ...
  doc.addPage();doc.setFillColor(...dark);doc.rect(0,0,210,297,"F");doc.setFillColor(...gold);doc.rect(0,0,4,297,"F");
  let sp=20;
  drawVMarkPDF(doc,M,sp-7,7,gold);
  doc.setTextColor(...gold);doc.setFontSize(9);doc.setFont("helvetica","bold");doc.text("VALORA",M+9,sp);
...
Finance: ${isHotelAdv?`${data.capStructure||"Single"} facility · Interest ${fmt(hotelAdv.interestTotal,currSym)} total hold`:`LTC ${data.ltc||"N/A"}%, All-in rate ${r.financeRate?(r.financeRate*100).toFixed(2)+"%":"N/A"}`}`.trim();
  ...
  if(loading)return(
    <div style={{minHeight:"100vh",background:"#0f1115",display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",gap:16}}>
      <div style={{display:"flex",alignItems:"center",gap:8}}>
      ...
      <div style={{width:32,height:32,border:"2px solid rgba(201,168,76,.15)",borderTopColor:"#c9993c",borderRadius:"50%",animation:"spin .7s linear infinite"}}/>
      ...
      <div style={{background:"var(--bg2)",borderBottom:"1px solid var(--border)",padding:"0 12px",display:"flex",alignItems:"center",gap:5,height:42,overflowX:"auto",flexShrink:0}}>
          ...
          <div style={{display:"flex",alignItems:"center",gap:6,flexShrink:0}}>
            ...
            <div style={{display:"flex",border:"1px solid var(--border)",borderRadius:6,overflow:"hidden"}}>
              {(["simple","advanced"] as const).map(m=>(
                <button key={m} onClick={()=>setHotelMode(m)} style={{padding:"3px 12px",background:hotelMode===m?"var(--gold)":"transparent",color:hotelMode===m?"#0f1115":"var(--text-d)",border:"none",fontSize:10,fontWeight:600,cursor:"pointer",fontFamily:"var(--font-body)",transition:"all .15s",whiteSpace:"nowrap"}}>
        ...
        {assetType==="Flip"&&(
          <div style={{display:"flex",alignItems:"center",gap:6,flexShrink:0}}>
            ...
            <div style={{display:"flex",border:"1px solid var(--border)",borderRadius:6,overflow:"hidden"}}>
              {(["simple","advanced"] as const).map(m=>(
                <button key={m} onClick={()=>setFlipComplexity(m)} style={{padding:"3px 12px",background:flipComplexity===m?"var(--gold)":"transparent",color:flipComplexity===m?"#0f1115":"var(--text-d)",border:"none",fontSize:10,fontWeight:600,cursor:"pointer",fontFamily:"var(--font-body)",transition:"all .15s",whiteSpace:"nowrap"}}>
                  {m==="simple"?"Simple":"Advanced"}
                </button>
              ))}
            </div>
                ...
                <div style={{background:"var(--gold-bg)",border:"1px solid var(--gold-border)",borderRadius:10,padding:14,marginBottom:20}}>
                  ...
                  <div style={{display:"flex",gap:8}}>
                    ...
                    <button onClick={handleUrlImport} disabled={urlImporting||!urlImport.trim()} className="btn-primary" style={{padding:"8px 14px",fontSize:12,flexShrink:0}}>
                      {urlImporting?<><span style={{width:10,height:10,border:"2px solid #c9993c22",borderTopColor:"#c9993c",borderRadius:"50%",display:"inline-block",animation:"spin .7s linear infinite"}}/>Importing…</>:"Import"}
                    </button>
                  </div>
                ...
                <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:20}}>
                  ...
                  <div style={{display:"flex",gap:0,borderRadius:6,border:"1px solid var(--border)",overflow:"hidden"}}>
                    {(["sqft","sqm"] as const).map(u=>(
                      <button key={u} onClick={()=>set("unitSystem",u)} style={{padding:"5px 14px",fontSize:11,fontFamily:"var(--font-mono)",background:(data.unitSystem||"sqft")===u?"var(--gold)":"var(--bg3)",color:(data.unitSystem||"sqft")===u?"#0f1115":"var(--text-m)",border:"none",cursor:"pointer",fontWeight:600,transition:"all .15s"}}>{u}</button>
                    ))}
                  </div>
                </div>
              ...
              <div>
                ...
                {(isPro||isTrialing)?(
                  ...
                  <div style={{marginBottom:16,padding:"12px 16px",background:"var(--bg3)",border:"1px solid var(--border)",borderRadius:6,textAlign:"center"}}>
                    ...
                    <button onClick={()=>router.push("/pricing")} style={{padding:"7px 20px",background:"var(--gold)",border:"none",borderRadius:5,color:"#0f1115",fontSize:11,fontWeight:600,cursor:"pointer",fontFamily:"var(--font-body)"}}>
                      Upgrade to Pro
                    </button>
                  </div>
                )}
      ...
      {brochureModal&&(
        <div className="modal-overlay" onClick={e=>{if(e.target===e.currentTarget)setBrochureModal(false);}}>
          <div className="modal" style={{width:660}}>
            ...
            {!brochureContent&&(<button className="btn-primary" onClick={generateBrochure} disabled={generatingBrochure} style={{width:"100%",justifyContent:"center",marginBottom:16,padding:"13px"}}>{generatingBrochure?<><span style={{width:14,height:14,border:"2px solid #c9993c22",borderTopColor:"#c9993c",borderRadius:"50%",display:"inline-block",animation:"spin .7s linear infinite"}}/> Analysing deal data…</>:"Generate AI Analysis →"}</button>)}
            ...
            {brochureContent&&!generatingBrochure&&(
              <div>
                ...
                <div style={{display:"flex",gap:10,marginTop:16}}>
                  ...
                  <button className="btn-primary" onClick={handleDownloadBrochure} disabled={downloadingBrochure} style={{flex:2,justifyContent:"center",padding:"12px"}}>{downloadingBrochure?<><span style={{width:14,height:14,border:"2px solid #c9993c22",borderTopColor:"#c9993c",borderRadius:"50%",display:"inline-block",animation:"spin .7s linear infinite"}}/> Generating PDF…</>:"⬇ Download Brochure PDF"}</button>
                </div>
  ...
  return(
    <Suspense fallback={
      <div style={{minHeight:"100vh",background:"#0f1115",display:"flex",alignItems:"center",justifyContent:"center"}}>
        <div style={{width:32,height:32,border:"2px solid rgba(201,153,60,.2)",borderTopColor:"#c9993c",borderRadius:"50%",animation:"spin .7s linear infinite"}}/>
        <style>{`@keyframes spin{to{transform:rotate(360deg)}}`}</style>
      </div>
    }>
      <AppraisalPage/>

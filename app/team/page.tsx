...
export default function TeamPage(){
  ...
  const load=async(uid:string)=>{
    ...
    // Also check if user owns a firm directly (owner may not have a firm_members row)
    const{data:ownedFirm}=await supabase.from("firms").select("*").eq("owner_id",uid).maybeSingle();
    const resolvedFirm=(myRow?.firms as any)||ownedFirm||null;
    if(resolvedFirm){
      const f=resolvedFirm;

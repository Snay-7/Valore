...
// ═════════════════════════════════════════════════════════════════════
// VALORA DASHBOARD v3 — Editorial refresh with dark + light mode
// Preserves current brand (Cormorant + Instrument Sans + gold #c9a84c)
// Adds: dual theme, refined spacing, cleaner cards, subtle depth
// ═════════════════════════════════════════════════════════════════════
...
/* ── LIGHT — Editorial cream, matching gold keeps premium feel ── */
body.light{
  --gold:#a8833a;--gold-l:#876724;--gold-bg:rgba(168,131,58,0.07);--gold-border:rgba(168,131,58,0.25);
  --bg:#faf8f3;--bg1:#ffffff;--bg2:#ffffff;--bg3:#f3efe7;--bg4:#ece6d7;
...
.card:hover{border-color:var(--gold-border);transform:translateY(-2px);box-shadow:var(--shadow-hover)}
.card.trashed{opacity:.6;border-style:dashed}

/* ── METRIC PANEL — nested sub-card for numbers ── */
.metrics-panel{
...
.metric-cell__label{
  font-size:9px;font-weight:600;color:var(--text-d);
...
.metric-pill{background:var(--bg3);border-radius:8px;padding:8px 10px}
...
body.light .modal-overlay{background:rgba(26,24,21,.45)}
.modal{background:var(--bg2);border:1px solid var(--border-m);border-radius:16px;padding:28px;width:460px;max-width:calc(100vw - 32px);box-shadow:0 20px 60px rgba(0,0,0,.3)}
...
.btn-danger{
  ...
  border:1px solid rgba(244,100,95,.3);border-radius:7px;
...
body.light .dropdown{box-shadow:0 8px 24px rgba(26,24,21,.12)}
.dropdown-item{
  display:flex;align-items:center;gap:8px;
  padding:8px 12px;border-radius:6px;font-size:12px;font-weight:500;
  cursor:pointer;transition:background .15s;
  width:100%;border:none;background:none;
  color:var(--text-m);font-family:var(--font-body);text-align:left;
}
...
.filter-tab{padding:10px 16px;font-size:11px;font-weight:500;background:none;border:none;border-bottom:2px solid transparent;color:var(--text-d);cursor:pointer;font-family:var(--font-body);transition:all .2s;white-space:nowrap;flex-shrink:0;letter-spacing:.08em;text-transform:uppercase;margin-bottom:-1px}
...
export default function Dashboard() {
  ...
  const createFromUrl = async () => {
    ...
    try {
      ...
      const { data: proj, error } = await supabase.from("projects").insert({
        name: projectName, location: d.location || "", asset_type: urlImportType, currency: urlImportCurrency,
        benchmark_rate: "SONIA", created_by: user.id, firm_id: null,
      }).select().single();
  ...
  return (
    <div style={{ minHeight: "100vh", background: "var(--bg)", color: "var(--text)", fontFamily: "var(--font-body)", display: "flex" }}>
      ...
      {/* ── MAIN ── */}
      <div className="main-content" style={{ marginLeft: 220, flex: 1, minWidth: 0, padding: "36px 36px", maxWidth: "calc(100vw - 220px)" }}>
        {/* Mobile top bar */}
        ...
        {view === "trash" && (
          <div>
            <div className="page-header">
              <div>
                <h1 style={{ fontFamily: "var(--font-display)", fontSize: 34, fontWeight: 300, marginBottom: 6, letterSpacing: ".01em" }}>Trash</h1>
              ...
              <div style={{ display: "flex", gap: 8 }}>
                ...
                {trashedProjects.length > 0 && <button className="btn-danger" onClick={() => setConfirmDelete({ type: "all" })} style={{ padding: "8px 16px", fontSize: 12 }}>Empty Trash</button>}
              </div>
            </div>
            {trashedProjects.length === 0 ? (
              <div style={{ textAlign: "center", padding: "80px 0" }}>
                <div style={{ fontSize: 48, marginBottom: 16, opacity: .3 }}>🗑</div>
                <p style={{ fontSize: 16, color: "var(--text-d)", fontFamily: "var(--font-display)", fontWeight: 300 }}>Trash is empty</p>
              </div>
            ) : (
                  ...
                  return (
                    <div key={p.id} className="card trashed" style={{ animationDelay: `${i * 0.04}s`, cursor: "default" }}>
                      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                        <span style={{ fontSize: 10, padding: "3px 10px", borderRadius: 5, background: "var(--gold-bg)", color: "var(--gold)", fontWeight: 600, border: "1px solid var(--gold-border)" }}>{ASSET_LABELS[p.asset_type]||p.asset_type}</span>
                        <span style={{ fontSize: 10, color: "var(--red)", fontFamily: "var(--font-mono)", fontWeight: 600 }}>Deletes in {p._daysLeft}d</span>
                      </div>
                      <div>
                        <h3 style={{ fontSize: 17, fontWeight: 500, marginBottom: 3, fontFamily: "var(--font-display)", letterSpacing: ".02em" }}>{p.name || "Untitled"}</h3>
                        <p style={{ fontSize: 12, color: "var(--text-m)" }}>{p.location || "No location"}</p>
                      </div>
                      {latest && (
                        <div className="metrics-panel" style={{ gridTemplateColumns: "1fr 1fr" }}>
                          <div className="metric-cell">
                            <div className="metric-cell__label">GDV</div>
                            <div className="metric-cell__value" style={{ color: "var(--gold)" }}>{fmt(latest.gdv, sym)}</div>
                          </div>
                          <div className="metric-cell">
        ...
        {view === "portfolio" && (
          <>
            <div className="page-header">
              <div>
                <h1 style={{ fontFamily: "var(--font-display)", fontSize: 34, fontWeight: 300, letterSpacing: ".01em", lineHeight: 1 }}>Portfolio</h1>
                {projects.length > 0 && (
              ...
              <div className="page-header-actions" style={{ display: "flex", flexDirection: "column", alignItems: "flex-end", gap: 4 }}>
                <div style={{ display: "flex", gap: 8 }}>
                  <button className="btn-ghost" style={{ borderColor: "var(--gold-border)", color: "var(--gold)" }} onClick={() => setShowUrlModal(true)}>
            ...
            {isTrialing && (
              <div style={{ background:"var(--gold-bg)", border:"1px solid var(--gold-border)", borderRadius:12, padding:"14px 18px", marginBottom:20, display:"flex", alignItems:"center", justifyContent:"space-between", gap:12, flexWrap:"wrap" }}>
                <div>
                  <div style={{ fontSize:13, fontWeight:600, color:"var(--gold)", marginBottom:2 }}>✦ Enterprise Trial — {trialDaysLeft} day{trialDaysLeft !== 1 ? "s" : ""} remaining</div>
                  <div style={{ fontSize:12, color:"var(--text-m)", fontWeight:500 }}>Full access to all features. Upgrade before your trial ends.</div>
                </div>
                <button className="btn-primary" style={{ padding:"8px 16px", fontSize:11, flexShrink:0 }} onClick={() => router.push("/pricing")}>Upgrade Now</button>
              </div>
            )}
            ...
            {!isPro && totalProjectCount >= activeProjectLimit && (
              <div style={{ background: "rgba(240,164,41,.06)", border: "1px solid rgba(240,164,41,.2)", borderRadius: 10, padding: "12px 18px", marginBottom: 16, display: "flex", justifyContent: "space-between", alignItems: "center", gap: 12, flexWrap: "wrap" }}>
            ...
            {projects.length === 0 && (
              <div style={{ textAlign: "center", padding: "80px 0" }}>
                ...
                <p style={{ fontSize: 17, color: "var(--text-m)", marginBottom: 8, fontFamily: "var(--font-display)", fontWeight: 400, letterSpacing: ".01em" }}>No projects yet</p>
                <p style={{ fontSize: 13, color: "var(--text-d)", marginBottom: 28, fontWeight: 500 }}>Create your first appraisal or book a demo to get started.</p>
                  ...
                  return (
                    <div key={p.id} className="card" style={{ animationDelay: `${i * 0.04}s` }} onClick={() => openProject(p)}>
                      ...
                      <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                        <span style={{ fontSize: 10, padding: "3px 10px", borderRadius: 5, background: "var(--gold-bg)", color: "var(--gold)", fontWeight: 600, letterSpacing: ".04em", border: "1px solid var(--gold-border)" }}>{ASSET_LABELS[p.asset_type]||p.asset_type}</span>
                        <span style={{ fontSize: 10, padding: "3px 10px", borderRadius: 5, background: "var(--bg3)", color: "var(--text-d)", fontWeight: 500, border: "1px solid var(--border)", textTransform: "capitalize" }}>{latest?.status || "draft"}</span>
                        <span style={{ fontSize: 10, color: "var(--text-d)", marginLeft: "auto", fontFamily: "var(--font-mono)", fontWeight: 500 }}>
                          {new Date(p.created_at).toLocaleDateString("en-GB", { day: "numeric", month: "short", year: "2-digit" })}
                        </span>
                      </div>
                      <div>
                        <h3 style={{ fontSize: 17, fontWeight: 500, marginBottom: 3, fontFamily: "var(--font-display)", letterSpacing: ".02em", lineHeight: 1.25 }}>{p.name || "Untitled"}</h3>
                        <p style={{ fontSize: 12, color: "var(--text-m)", fontWeight: 500 }}>{p.location || "No location set"}</p>
                      </div>
                      {latest ? (
                        ...
                        <div style={{ background: "var(--bg3)", borderRadius: 8, padding: "10px 14px", fontSize: 12, color: "var(--text-d)", fontWeight: 500 }}>No appraisal saved yet</div>
                      )}
                      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", paddingTop: 4, fontSize: 11, color: "var(--text-d)", fontWeight: 500 }}>
          ...
          <div className="modal-overlay" onClick={e => { if (e.target === e.currentTarget) { setShowUrlModal(false); setUrlImportError(null); } }}>
            <div className="modal" style={{ width: 500 }}>
              <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 8 }}>
                ...
                <div style={{ fontFamily: "var(--font-display)", fontSize: 26, fontWeight: 300, letterSpacing: ".01em" }}>Import from URL</div>
              </div>
              <p style={{ fontSize: 12, color: "var(--text-d)", marginBottom: 22, fontWeight: 500, lineHeight: 1.6 }}>
        ...
        {showNewModal && (
          <div className="modal-overlay" onClick={e => { if (e.target === e.currentTarget) setShowNewModal(false); }}>
            <div className="modal">
              <div style={{ fontFamily: "var(--font-display)", fontSize: 26, fontWeight: 300, marginBottom: 4, letterSpacing: ".01em" }}>New Appraisal</div>
              <p style={{ fontSize: 13, color: "var(--text-d)", marginBottom: 22, fontWeight: 500 }}>Set up a new project to get started.</p>
              <div className="inp-group">
        ...
        {confirmDelete && (
          <div className="modal-overlay" onClick={e => { if (e.target === e.currentTarget) setConfirmDelete(null); }}>
            <div className="modal" style={{ width: 420 }}>
              <div style={{ fontFamily: "var(--font-display)", fontSize: 24, fontWeight: 300, marginBottom: 8, color: "var(--red)", letterSpacing: ".01em" }}>
              ...
              <div style={{ display: "flex", gap: 8 }}>
                ...
                <button onClick={() => confirmDelete.type === "all" ? emptyTrash() : permanentlyDelete(confirmDelete.project.id)}
                  style={{ flex: 1, background: "var(--red)", color: "#fff", border: "none", borderRadius: 8, padding: "10px 18px", fontFamily: "var(--font-body)", fontSize: 13, fontWeight: 600, cursor: "pointer" }}>

          ...
          <div style={{padding:24,overflowY:"auto",flex:1}} className="editor-pad">
            ...
            {activeTab==="cashflow"&&assetType==="Flip"&&(()=>{
              const flipCfs:number[]=r.flipMode==="hold"
                ?[-((r.equityIn||r.equity)||0),...Array(Math.max(0,Math.round(r.bridgingMonths||0)-1)).fill(0),...Array(Math.max(0,Math.round(r.refiMonths||0)-1)).fill(r.netCashflowPm||0),(r.netCashflowPm||0)+((r.netProceeds||0)-(r.refiLoan||0))]
                :[-((r.equityIn||r.equity)||0),...Array(Math.max(0,Math.round(r.bridgingMonths||0)-1)).fill(0),(r.netProceeds||0)-(r.peakLoanBalance||0)];
              const totalMos=r.flipMode==="hold"?(r.totalHoldMonths||r.bridgingMonths||6):(r.bridgingMonths||6);
              return(
                <div>
                  <div className="section-title">Cashflow</div>
                  <div style={{fontSize:12,color:"var(--text-d)",marginBottom:16}}>
                  ...
                  <CashflowChart uCfs={flipCfs} totalMonths={totalMos} buildMonths={r.bridgingMonths||0} exitLabel={r.flipMode==="hold"?"Sale + refi repaid":"Net sale proceeds"} currencySymbol={currencySymbol} phaseLabels={r.flipMode==="hold"?[`Bridge (${r.bridgingMonths}m)`,`Hold (${r.refiMonths}m)`]:undefined}/>
                </div>
              );
            })()}

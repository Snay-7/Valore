...
.btn-primary{background:var(--gold);color:#0f1115;border:none;border-radius:7px;padding:9px 18px;font-family:var(--font-body);font-size:12px;font-weight:600;cursor:pointer;transition:background .2s}
...
.nav-item.active{color:var(--gold);background:rgba(201,153,60,.08);border-color:var(--gold-border);font-weight:600}
...
export default function Dashboard() {
  ...
  if (loading) return (
    <div style={{ minHeight: "100vh", background: "#0f1115", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", gap: 14 }}>
      ...
      <div style={{ width: 26, height: 26, border: "2px solid rgba(201,153,60,.15)", borderTopColor: "#c9993c", borderRadius: "50%", animation: "spin .7s linear infinite" }} />
      ...
      <div className="main-content" style={{ marginLeft: 210, flex: 1, minWidth: 0, padding: "32px 32px", maxWidth: "calc(100vw - 210px)" }}>
        ...
        {view === "portfolio" && (
          <>
            ...
            {/* Trial banner */}
            {isTrialing && (
              <div style={{ background:"rgba(201,153,60,.08)", border:"1px solid var(--gold-border)", borderRadius:10, padding:"12px 16px", marginBottom:18, display:"flex", alignItems:"center", justifyContent:"space-between", gap:12, flexWrap:"wrap" }}>
                <div>
            ...
            <div className="modal" style={{ width: 500 }}>
              ...
              <div style={{ display: "flex", gap: 8 }}>
                ...
                <button className="btn-primary" onClick={createFromUrl} disabled={!urlImport.trim() || urlImporting} style={{ flex: 2, opacity: !urlImport.trim() ? 0.5 : 1, display: "flex", alignItems: "center", justifyContent: "center", gap: 8 }}>
                  {urlImporting ? (
                    <><span style={{ width: 12, height: 12, border: "2px solid rgba(6,7,10,.3)", borderTopColor: "#0f1115", borderRadius: "50%", display: "inline-block", animation: "spin 0.8s linear infinite" }} />Importing…</>
                  ) : "Import & Open →"}
                </button>
              </div>
            </div>

...
export default function ProjectDetailPage() {
  ...
  <div style={{ background:"var(--bg3)", border:"1px solid var(--border)", borderRadius:8, padding:"8px 4px", marginTop:6 }}>
    ...
    {/* Everyone option */}
    <div className="member-check" onClick={() => setNoteVisibility([])} style={{ cursor:"pointer" }}>
      <div style={{ width:16, height:16, borderRadius:3, border:`2px solid ${noteVisibility.length===0?"var(--gold)":"var(--border-m)"}`, background:noteVisibility.length===0?"var(--gold)":"transparent", display:"flex", alignItems:"center", justifyContent:"center", flexShrink:0 }}>
        {noteVisibility.length===0 && <span style={{ color:"#06070a", fontSize:10, fontWeight:700 }}>✓</span>}
      </div>
    ...
    {/* Individual members */}
    {otherMembers.map(m => {
      const checked = noteVisibility.length===0 || noteVisibility.includes(m.user_id);
      return (
        <div key={m.id} className="member-check" style={{ cursor:"pointer" }} onClick={() => {
          if (noteVisibility.length===0) {
            setNoteVisibility(otherMembers.filter(x => x.user_id !== m.user_id).map(x => x.user_id));
          } else {
            setNoteVisibility(prev => prev.includes(m.user_id) ? prev.filter(id => id !== m.user_id) : [...prev, m.user_id]);
          }
        }}>
          <div style={{ width:16, height:16, borderRadius:3, border:`2px solid ${checked?"var(--gold)":"var(--border-m)"}`, background:checked?"var(--gold)":"transparent", display:"flex", alignItems:"center", justifyContent:"center", flexShrink:0 }}>
            {checked && <span style={{ color:"#06070a", fontSize:10, fontWeight:700 }}>✓</span>}
          </div>

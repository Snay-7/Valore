...
:root{
  --gold:#c9a84c;--gold-l:#e2c97e;--gold-bg:rgba(201,168,76,0.06);--gold-border:rgba(201,168,76,0.2);
  --bg:#06070a;--bg1:#08090d;--bg2:#0d1016;--bg3:#12151c;--bg4:#181b24;
  --text:#ede9e0;--text-m:#7a8390;--text-d:#363c46;
  --border:rgba(255,255,255,0.05);--border-m:rgba(255,255,255,0.09);
...
.metric:hover{border-color:rgba(201,168,76,.12);transform:translateY(-1px)}
...
.act.open{border-color:var(--gold-border);background:rgba(201,168,76,.03)}
...
/* ── Buttons ── */
.btn-gold{background:var(--gold);color:#06070a;border:none;border-radius:9px;padding:10px 20px;font-family:var(--font-body);font-size:13px;font-weight:600;cursor:pointer;transition:opacity .2s}
...
export default function WorkspaceProjectPage() {
  ...
  if (loading) return (
    <div style={{minHeight:"100vh",background:"#06070a",display:"flex",alignItems:"center",justifyContent:"center"}}>
      <div style={{width:30,height:30,border:"2px solid rgba(201,168,76,.1)",borderTopColor:"#c9a84c",borderRadius:"50%",animation:"spin .8s linear infinite"}}/>
  ...
  return (
    <div style={{minHeight:"100vh",background:"var(--bg)",color:"var(--text)",fontFamily:"var(--font-body)"}}>
      ...
      <div style={{position:"fixed",inset:0,pointerEvents:"none",zIndex:0,
        background:"radial-gradient(ellipse 60% 40% at 15% 60%,rgba(201,168,76,.03) 0%,transparent 60%), radial-gradient(ellipse 50% 50% at 85% 20%,rgba(91,156,246,.025) 0%,transparent 55%)"}}/>
      ...
      <div style={{transition:"padding-right .32s cubic-bezier(.16,1,.3,1)",paddingRight:isOpen?"var(--panel-w)":0,position:"relative",zIndex:1}}>
        <div style={{maxWidth:840,margin:"0 auto",padding:"48px 28px 80px"}}>
          ...
          {/* Gold rule */}
          <div style={{height:1,background:"linear-gradient(90deg,rgba(201,168,76,.3),rgba(201,168,76,.05) 60%,transparent)",marginBottom:36,animation:"fadeUp .35s .04s ease both",opacity:0,animationFillMode:"forwards"}}/>
          ...
          <div style={{display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:14,marginBottom:44,animation:"fadeUp .35s .18s ease both",opacity:0,animationFillMode:"forwards"}}>
            ...
            <button className="act" onClick={()=>router.push(appr?`/appraisal?project=${pid}&appraisal=${appr.id}`:`/appraisal?project=${pid}`)}>
              <div className="act-icon" style={{background:"linear-gradient(135deg,rgba(201,168,76,.18),rgba(201,168,76,.06))",border:"1px solid var(--gold-border)"}}>

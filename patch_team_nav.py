...
# Fix: Replace desktop nav with one that works on mobile
old_nav = '''      {/* Nav */}
...
new_nav = '''      {/* Nav */}
      <nav style={{background:"var(--bg1)",borderBottom:"1px solid var(--border)",padding:"0 16px",height:52,display:"flex",alignItems:"center",gap:8,position:"sticky",top:0,zIndex:40}}>
        ...
        <div style={{flex:1}}/>
        <button onClick={()=>router.push("/dashboard")} className="btn-ghost" style={{fontSize:11,padding:"4px 10px"}}>← Back</button>
        <button onClick={async()=>{await supabase.auth.signOut();router.push("/");}} className="btn-ghost" style={{fontSize:11,padding:"4px 10px"}}>Sign Out</button>
      </nav>'''
